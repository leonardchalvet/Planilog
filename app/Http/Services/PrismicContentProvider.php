...
class PrismicContentProvider
{
     ...
     * @return mixed
     */
    public function getBackpageContent($slug, $type, $document)
    {
        if (null == $document) {
            abort(404, "backpage $slug not found");
        }

        $document->data->{$type}->uid = $document->uid;
        $document->data->{$type}->tags = $document->tags;
        $document->data->{$type}->lang = substr($document->lang, 0, 2);
     ...
     * @return mixed
     */
    public function getSoftwares(string $locale, ?array $params = [])
    {
        $response = $this->get('software', $locale, [
            "limit" => $params['limit'] ?? null,
            "page" => $params['page'] ?? null,
            "search" => $params['search'] ?? null,
            "filters" => $params['filters'] ?? null
        ...
        if (count($response->results) == 0) {
            Debugbar::warning("No software ($locale) found");
        }
     ...
     * @return mixed
     */
    public function getSoftware($slug)
    {
        /** @var Document $document */
        Debugbar::startMeasure('prismic', "Prismic API [software: $slug]");
        $document = $this->api->getByUID('software', $slug);
        Debugbar::stopMeasure('prismic');

        if (null == $document) {
            abort(404, "Software $slug not found");
        }

        $document->data->software->uid = $document->uid;
        $document->data->software->tags = $document->tags;
        $document->data->software->lang = substr($document->lang, 0, 2);
        return $document->data->software;
    }
}
